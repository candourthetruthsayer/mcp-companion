...
// Each entry lists the file and the key that holds the server map:
//   mcpServers    — Claude Code / Cursor / Windsurf / VS Code / generic
//   context_servers — Zed
const CONFIG_SOURCES = [
  { name: '.mcp.json', serverKey: 'mcpServers' },
  { name: 'mcp.json', serverKey: 'mcpServers' },
  { name: '.cursor/mcp.json', serverKey: 'mcpServers' },
  { name: '.vscode/mcp.json', serverKey: 'mcpServers' },
  { name: 'mcp_config.json', serverKey: 'mcpServers' }, // Windsurf
  { name: 'settings.json', serverKey: 'context_servers' }, // Zed — only if key present
];
...
// `serverUrl` (Windsurf) is resolved as an alias for `url` inside checkServer.
...
// Reasonably strict env-var reference check: ${VAR}, ${VAR:-default},
// or Windsurf's ${env:VAR} interpolation syntax.
function referencesEnv(value) {
  return /\$\{(env:)?[A-Za-z_][A-Za-z0-9_]*(:-[^}]*)?\}/.test(String(value));
}

async function findConfigs(dir) {
  const found = [];
  for (const { name, serverKey } of CONFIG_SOURCES) {
    const full = path.join(dir, name);
    try {
      const stat = await fs.stat(full);
      if (stat.isFile()) found.push({ name, path: full, source: 'project', serverKey });
    } catch {
      // not present — ignore
    }
  }
  ...
  try {
    ...
    if (stat.isFile()) found.push({ name: 'managed-mcp.json', path: managed, source: 'managed', serverKey: 'mcpServers' });
  } catch {
    /* ignore */
  }
  return found;
...
async function parseFile(file) {
  ...
  const serverKey = file.serverKey || 'mcpServers';
  const servers = data[serverKey];
  if (typeof servers !== 'object' || servers === null || Array.isArray(servers)) {
    // Zed's settings.json is only an MCP config when context_servers is present —
    // a bare settings.json is not an error, just not an MCP source.
    if (serverKey === 'context_servers') {
      return { file, error: null, skipped: true };
    }
    return { file, error: `missing "${serverKey}" object` };
  }
  return { file, data, serverKey };
}
...
function checkServer(name, cfg, file) {
  ...
  if (rawType === undefined) {
    // Claude Desktop / Windsurf / generic configs may omit "type":
    // a "command" implies stdio; a "url"/"serverUrl" implies a remote transport.
    if (cfg.command) {
      findings.transport = 'stdio';
      findings.issues.push({ severity: 'info', code: 'inferred-stdio',
        message: 'no "type" field — assuming stdio because "command" is present' });
    } else if (cfg.url || cfg.serverUrl) {
      findings.transport = 'http';
      findings.issues.push({ severity: 'info', code: 'inferred-http',
        message: 'no "type" field — assuming http because "url"/"serverUrl" is present' });
    } else {
      findings.transport = 'unknown';
      findings.issues.push({ severity: 'error', code: 'missing-type',
        message: 'no "type" field and no "command"/"url" to infer transport' });
    }
  } else {
  ...
  // Required fields per transport. `serverUrl` is Windsurf's alias for `url`.
        ...
        if (field === 'url') {
          ...
          if (urlValue === undefined || urlValue === null || urlValue === '') {
            findings.issues.push({ severity: 'error', code: 'missing-field',
              message: `${findings.transport} transport requires a non-empty "url" field` });
          } else {
            findings.url = String(urlValue);
          }
        } else {
          const v = cfg[field];
...
async function scan(dir, opts = {}) {
  ...
  for (const file of configs) {
    ...
    if (parsed.skipped) continue; // e.g. Zed settings.json without context_servers
    const serverKey = parsed.serverKey || 'mcpServers';
    const servers = Object.entries(parsed.data[serverKey] || {}).map(([name, cfg]) => checkServer(name, cfg, file));
