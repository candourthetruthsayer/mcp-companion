...
function checkServer(name, cfg, file) {
  const issues = [];
  const findings = { name, issues: [], transport: 'unknown', command: null, url: null, packageNames: [], packageRefs: [] };
...
async function scan(dir, opts = {}) {
  ...
  return { projectRoot: root, configs: results, summary, checkUpdates, checkHealth };
}
